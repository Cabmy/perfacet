#!/usr/bin/env python3
"""HTTP 压测：连已启动的 Perfacet 或直连 mock。测客户端 RTT 与 RPS。"""
...
async def one(
    session: aiohttp.ClientSession, url: str, token: str, i: int, name: str
) -> float:
    headers = {
...
async def run(url: str, token: str, n: int, concurrency: int, name: str) -> None:
    lat: list[float] = []
    ...
    async def bound(i: int) -> None:
        async with sem:
            ms = await one(session, url, token, i, name)
            lat.append(ms)
...
def main() -> None:
    ...
    ap.add_argument("--name", default="echo__echo")
    ap.add_argument("-n", type=int, default=8000)
    ap.add_argument("-c", type=int, default=32)
    args = ap.parse_args()
    asyncio.run(run(args.url, args.token, args.n, args.c, args.name))
